...
const LoginForm:React.FC<InjectedFormProps< loginFormValuesType, LoginFormOwnProps > & LoginFormOwnProps > = ({ handleSubmit, error, captchaUrl }) => {
    return (
        <form onSubmit={handleSubmit}>
            {сreateField<loginFormValuesTypeKeys>("Email", "email", [required], Input)}
            {сreateField<loginFormValuesTypeKeys>("Password", "password", [required], Input, { type: "password" })}
            {сreateField<loginFormValuesTypeKeys>(undefined, "rememberMe", [], Input, { type: "checkbox" }, "Remember me")}

            {captchaUrl && <div className={s.cap}><img src={captchaUrl} /></div>}
            {captchaUrl && сreateField<loginFormValuesTypeKeys>( "simbols from captcha", "captcha", [required], Input, {} )}
